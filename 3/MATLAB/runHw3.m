...
signAcademicHonestyPolicy('Yuhao Liu', 'liu697');
...
[src_x, src_y] = getPointsFromUser(portrait_img, 4, 'Click any 4 points');
[dst_x, dst_y] = getPointsFromUser(bg_img, 4, 'Click the corresponding locations of the previous 4 points');

portrait_pts = [src_x, src_y];
bg_pts = [dst_x, dst_y];
...
% Your own panorama
% % stitch three images
imgc = im2single(imread('apt_center.jpeg'));
imgl = im2single(imread('apt_left.jpeg'));
imgr = im2single(imread('apt_right.jpeg'));
...
%figure, imshow(stitched_img);
imwrite(stitched_img, 'apt_panorama.png');
