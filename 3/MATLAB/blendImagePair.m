...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            wmasks = (bwdist(~binary_mask_s,'euclidean'));
            wmaskd = (bwdist(~binary_mask_d,'euclidean'));
            channel_out = (wmasks .* wrapped_imgs(:, :, c) + wmaskd .* wrapped_imgd(:, :, c)) ./ (wmasks + wmaskd);
            channel_out(~(binary_mask_s | binary_mask_d)) = 0.0;
        end
