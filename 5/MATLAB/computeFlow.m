function result = computeFlow(img1, img2, search_radius, template_radius, grid_MN)
    ...
    for i = 1:M
        for j = 1:N
            ...
            % search_area = img2(max(row-search_radius, 1): min(row+search_radius, H), max(1, col-search_radius): min(col+search_radius, W));
            search_area = img2(max(row-template_radius-search_radius, 1): min(row+template_radius+search_radius, H), max(1, col-template_radius-search_radius): min(col+template_radius+search_radius, W));
            % compute correlation
            ...
            U(i, j) = max_ind_col - (min(search_radius, col-template_radius) + 2*template_radius+1);
            V(i, j) = max_ind_row - (min(search_radius, row-template_radius) + 2*template_radius+1);
        end
    end
