function trackingTester(data_params, tracking_params)
    ...
    for frame_id = frame_ids(2:end)
        ...
        search_window = frame(max(obj_row-tracking_params.search_radius, 1): min(obj_row+obj_height+tracking_params.search_radius, H), ...
            max(obj_col-tracking_params.search_radius, 1): min(obj_col+obj_width+tracking_params.search_radius, W), :);
        %------------- END FILL IN CODE -----------------
        % Change to grayscale
        ...
        % tracking.
%         cor_r = zeros(num_windows, 1);
%         for i = 1:num_windows
%              cor_r(i) = corr(obj_hist.', candidate_hists(:,i));
%         end
%         [argvalue, argmax] = max(cor_r);
%         
%         errs = zeros(num_windows, 1);
%         for i = 1:num_windows
%              errs(i) = immse(obj_hist.', candidate_hists(:,i));
%         end
%         err_map = reshape(errs, [width_temp, width_temp]);
        
        width_temp = size(search_window, 2) - obj_width + 1;
        ...
        err_map = reshape(errs, [width_temp, width_temp]);
        [argvalue, argmin] = min(errs);
        [I1,I2] = ind2sub(size(err_map),argmin);

%         cor_r = zeros(num_windows, 1);
%         for i = 1:num_windows
%              cor_r(i) = corr(obj_hist.', candidate_hists(:,i));
%         end
%         cor_map = reshape(cor_r, [width_temp, width_temp]);
%         [argvalue, argmax] = max(cor_r);
%         [I1,I2] = ind2sub(size(cor_map),argmax);
        
        obj_row = obj_row - width_temp/2 + I1;
        obj_col = obj_col - width_temp/2 + I2;
